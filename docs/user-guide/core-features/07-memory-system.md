...
> **Storage Architecture**: All memory paths are managed through the centralized [Storage Router](../getting-started/03-project-structure.md#storage-router-architecture). User memories are isolated per-profile at `profiles/<username>/memory/`. The storage router ensures consistent path resolution across all components.

### Types of Memory

#### Episodic Memory (`profiles/<username>/memory/episodic/YYYY/*.json`)
...
#### Task Memory (`profiles/<username>/memory/tasks/`)
...
**Architecture:**
```
profiles/<username>/memory/functions/
...
- **Access**: Via blue pencil icon (✏️) next to any memory entry in any tab
...
#### 5. Tasks Tab ✓
- **Purpose**: View all task files from `profiles/<username>/memory/tasks/`
...
```bash
# Place files in the inbox (for CLI users without profile context)
cp my-notes.txt profiles/<username>/memory/inbox/
...
# OR place audio files in profiles/<username>/memory/audio/inbox/
...
This:
1. Reads all episodic memories from `profiles/<username>/memory/episodic/`
2. Generates embeddings using Ollama (`nomic-embed-text` model)
3. Stores vectors in `profiles/<username>/memory/index/memory-index.json`
