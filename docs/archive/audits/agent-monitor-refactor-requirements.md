...
> service; see the historical record at
> `docs/archive/plans-and-migrations/ENVIRONMENT_INTERFACE_BRIDGE.md`.
...
- Migrated `MemoryControls.svelte` and `TaskManager.svelte` from `/api/agent` to `/api/agents/run`, preserving CLI option arguments for curator runs.
...
- Removed `headless-watcher` from process resolver maps in:
  ...
  - `packages/core/src/agent-bootstrap.ts`.
...
- Updated `packages/core/src/agent-bootstrap.ts` to use the shared agent executable resolver, removing its duplicate local service override, agent script override, and path-resolution logic.
