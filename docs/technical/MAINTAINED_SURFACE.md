...
- `packages/core/src/active-operator/mode-controller.ts` owns Reactive, Semi, and Full mode transitions. Semi owns configured scheduled and idle-triggered awake work. In Full, Robot Operator admits the next robot workflow only after the current correlated execution/result chain finishes; the retired general Operator Policy loop must not be reintroduced as a competing producer.
