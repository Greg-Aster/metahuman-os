...
- `brain/services/robot-operator.ts` owns Boredom Observer, Movement, and Reflection timing and admission. Their specialized planner graphs each use bounded persona, context, and current trigger material to author a high-level intention for the editable Boredom Autonomy executor. That executor reuses Environment Task State, capability admission, bridge transport, and correlated feedback rather than adding a scheduler, queue, state store, or hidden action path. Robot Operator remains dormant in Reactive mode or while Sleep is active.
