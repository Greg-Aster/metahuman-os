...
function readModelRegistry() {
  try {
    const p = path.join(paths.root, 'etc', 'models.json')
    return JSON.parse(fs.readFileSync(p, 'utf-8'))
  } catch { return { globalSettings: {}, defaults: {}, models: {} } }
}
...
export const GET: APIRoute = async () => {
  try {
    ...
    return new Response(JSON.stringify({ success: true, agent: globalSettings, baseModels, loras, active, dualAvailable }), { status: 200, headers: { 'Content-Type': 'application/json' } })
  } catch (e) {
...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    return new Response(JSON.stringify({ success: true, agent: registry.globalSettings || {} }), { status: 200, headers: { 'Content-Type': 'application/json' } })
  } catch (e) {
    return new Response(JSON.stringify({ success: false, error: (e as Error).message }), { status: 500, headers: { 'Content-Type': 'application/json' } })
  }
}
