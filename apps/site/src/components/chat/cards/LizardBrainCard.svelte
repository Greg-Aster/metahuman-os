    ...
    {:else if pendingProposal}
      <div class="mt-3 pt-3 border-t border-white/10">
        ...
        <div class="flex gap-2 flex-wrap">
          <button
            class="lizard-btn bg-green-500 text-white hover:bg-green-600"
            disabled={loading}
          ...
          <button
            class="lizard-btn bg-red-500/20 text-red-400 border border-red-500/30 hover:bg-red-500/30"
            disabled={loading}
          ...
          <button
            class="lizard-btn text-blue-400 border border-blue-500/30 hover:bg-blue-500/30 {showModifyInput ? 'bg-blue-500/40' : 'bg-blue-500/20'}"
            disabled={loading}
        ...
        {#if showModifyInput}
          <div class="mt-3 p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg">
            ...
            <div class="flex gap-2 mt-2 justify-end">
              <button
                class="lizard-btn bg-blue-500 text-white hover:bg-blue-600"
                disabled={!modifyInput.trim() || loading}
                on:click={submitModification}
              >
                {loading ? 'Submitting...' : 'Submit Modification'}
              </button>
              <button class="lizard-btn bg-transparent text-gray-400 border border-gray-600 hover:bg-white/5" on:click={toggleModify}>
                Cancel
              </button>
            </div>
          </div>
