...
# - AUTO-INCREMENTS versionCode on every build
#
...
if [ -f "$VERSION_FILE" ]; then
    CURRENT_VERSION=$(jq -r '.version // "1.0.0"' "$VERSION_FILE" 2>/dev/null || echo "1.0.0")
    CURRENT_VERSION_CODE=$(jq -r '.versionCode // 0' "$VERSION_FILE" 2>/dev/null || echo "0")
else
...
BUILD_NOTES="Build $NEW_VERSION_CODE - $BUILD_DATE"

echo "[Version] Current: v$CURRENT_VERSION (code: $CURRENT_VERSION_CODE)"
echo "[Version] Building: v$NEW_VERSION (code: $NEW_VERSION_CODE)"
echo ""
...
cp "$APK_PATH" "$APK_VERSIONED"
cp "$APK_PATH" "$APK_LATEST"

# Also copy to public downloads directory for server hosting
DOWNLOADS_DIR="$ROOT_DIR/apps/site/public/downloads"
mkdir -p "$DOWNLOADS_DIR"
cp "$APK_PATH" "$DOWNLOADS_DIR/metahuman-os.apk"

# Update version.json with final file size
cat > "$VERSION_FILE" << EOF
{
  "version": "$NEW_VERSION",
  "versionCode": $NEW_VERSION_CODE,
...
echo "=========================================="
echo ""
echo "  Version: v$NEW_VERSION (code: $NEW_VERSION_CODE)"
echo "  APK: $APK_PATH"
echo "  Size: $APK_SIZE"
echo ""
echo "  Releases:"
...
echo ""
echo "  Mobile devices will see this as an available update!"
echo ""
