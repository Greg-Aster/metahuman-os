#!/usr/bin/env node
// Main command dispatcher. The lightweight help-only launcher lives in entry.ts.
...
import {
  ...
  ollamaCommand,
  vllmCommand,
} from './commands/model-backends.js';
...
async function main() {
  ...
  let userContext: Parameters<typeof withUserContext>[0] | null = null;
  if (username) {
    const user = getUserByUsername(username);
    if (!user) {
      console.error(`Error: User '${username}' not found`);
  ...
  const executeCommand = async () => {
    try {
      ...
      case 'ingest':
        ...
        break;
      case 'ollama':
        await ollamaCommand(args);
        break;
      case 'vllm':
        await vllmCommand(args);
        break;
      case 'backend':
        await backendCommand(args);
        break;
      case 'guide':
