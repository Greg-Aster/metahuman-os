 ...
 */
export async function handleGetVoiceSettings(req: UnifiedRequest): Promise<UnifiedResponse> {
  const { user } = req;
  ...
  try {
    ...
    return errorResponse('Failed to load voice settings', 500);
  }
}

export async function handleSaveVoiceSettings(req: UnifiedRequest): Promise<UnifiedResponse> {
  const { user } = req;
  ...
  try {
    ...
    const storageResult = storageClient.getProfileRoot(user.username);
    if (!storageResult.success || !storageResult.profileRoot) {
      return errorResponse('Failed to resolve profile storage', 500);
    }
    ...
    if (sovits) {
      ...
      const sovitsConfig = config.tts.sovits!;
      if (sovits.serverUrl) sovitsConfig.serverUrl = sovits.serverUrl;
      if (sovits.speakerId) sovitsConfig.speakerId = sovits.speakerId;
      if (typeof sovits.temperature === 'number') sovitsConfig.temperature = sovits.temperature;
      if (typeof sovits.speed === 'number') sovitsConfig.speed = sovits.speed;
      if (typeof sovits.autoFallbackToPiper === 'boolean') {
        sovitsConfig.autoFallbackToPiper = sovits.autoFallbackToPiper;
      }
    }
    ...
    if (kokoro) {
      ...
      const kokoroConfig = config.tts.kokoro!;
      if (kokoro.langCode) kokoroConfig.langCode = kokoro.langCode;
      ...
      if (kokoro.voice) {
        if (kokoro.voice.startsWith('custom_')) {
          ...
          kokoroConfig.useCustomVoicepack = true;
          kokoroConfig.customVoicepackPath = voicepackPath;
          // Keep the voice ID without the custom_ prefix for display
          kokoroConfig.voice = voicepackName;
        } else {
          // Built-in voice selected
          kokoroConfig.voice = kokoro.voice;
          kokoroConfig.useCustomVoicepack = false;
        }
      }

      const parsedSpeed = Number(kokoro.speed);
      if (Number.isFinite(parsedSpeed)) {
        kokoroConfig.speed = clamp(parsedSpeed, 0.5, 2.0);
      }
      if (typeof kokoro.autoFallbackToPiper === 'boolean') {
        kokoroConfig.autoFallbackToPiper = kokoro.autoFallbackToPiper;
      }
    ...
    return errorResponse('Failed to save voice settings', 500);
  }
}
