 ...
 * If users.ts has not registered its resolver yet, read the canonical user
 * registry directly so custom-storage safety rules still apply.
 */
function getProfileStorageConfigLazy(username: string): ProfileStorageConfigFull | undefined {
  if (!_getProfileStorageConfig) {
    try {
      ...
      console.error('[path-builder] Failed to read profile storage configuration:', error);
    }
    return undefined;
  }
