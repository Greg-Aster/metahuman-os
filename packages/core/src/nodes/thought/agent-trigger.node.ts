...
export const AgentTriggerNode: NodeDefinition = defineNode({
  ...
  outputs: [
    ...
    { name: 'probability', type: 'number', optional: true, description: 'Configured admission probability' },
    { name: 'roll', type: 'number', optional: true, description: 'Deterministic admission roll' },
    { name: 'agentName', type: 'string', optional: true, description: 'Target follow-on agent' },
  ],
  properties: {
    agentName: 'train-of-thought',
    sourceAgent: 'cognitive-graph',
    probability: 0.2,
  },
  propertySchemas: {
    agentName: {
      ...
      description: 'Agent workflow to submit when the probabilistic gate admits this seed.',
      required: true,
    },
    sourceAgent: {
      type: 'text',
      default: 'cognitive-graph',
      label: 'Source Agent',
      description: 'Kebab-case producer identity recorded on the submitted follow-on work.',
      required: true,
    },
    probability: {
      type: 'slider',
      ...
      description: 'Chance from 0 to 1 that this node admits follow-on work for a unique seed.',
      min: 0,
      max: 1,
      step: 0.05,
    },
