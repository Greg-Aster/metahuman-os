...
const LOCK_NAME = 'agent-desire-executor';

async function main() {
  initGlobalLogger('desire-executor');
  if (isLocked(LOCK_NAME)) { console.log('[desire-executor] Another instance running. Exiting.'); process.exit(0); }
  if (!acquireLock(LOCK_NAME)) { console.log('[desire-executor] Failed to acquire lock. Exiting.'); process.exit(0); }

  const args = process.argv.slice(2);
  const options: DesireExecutorOptions = { singleUser: args.includes('--single-user') };
