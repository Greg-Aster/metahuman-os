...
function collectEpisodicMemories(username?: string): MemoryRecord[] {
  ...
  let root: string;
  if (username) {
    root = getProfilePaths(username).episodic;
...
function styleGuideForMemory(mem: MemoryRecord): string {
  switch ((mem.type || '').toLowerCase()) {
    case 'reflection':
      return 'Provide a concise (2-3 sentences) first-person insight capturing the main takeaway. Avoid meta commentary.';
    case 'reflection_summary':
      return 'Deliver a short conclusion highlighting the key lesson in 1-2 sentences.';
    case 'conversation':
    case 'chat':
      return 'Answer the implied user request directly in Greg\'s voice, under four sentences, without revealing internal reasoning.';
    case 'inner_dialogue':
      return 'Convert the inner thought into a resolved first-person statement or decision within 2 sentences.';
    case 'dream':
      return 'Summarize the dream scenario in 2-3 vivid sentences, focusing on feelings and imagery without analysis.';
    case 'observation':
    case 'journal':
      return 'Offer a succinct reflection or plan (2-3 sentences) responding to the observation, keeping it actionable.';
    case 'action':
      return 'Explain the action taken and the next step in 2 sentences, emphasizing purpose.';
    case 'audio':
    case 'audio_transcript':
      return 'Paraphrase the audio insight in first person, max two sentences, highlighting the core information.';
    default:
      return 'Produce a direct first-person response in 2-3 sentences, free of filler and markdown.';
  }
}
...
function buildFallbackSample(mem: any, batchIndex: number): GeneratedSample {
  ...
  for (let index = 0; index < batches.length; index += 1) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      ...
      if (attempt > 1) {
        ...
        messages.push({
          role: 'user',
          content: 'Repeat the task strictly following the requirements. Return ONLY the JSON object with sanitized, concise outputs that honour each styleGuide.',
        });
      }
    ...
    if (!parsed || !Array.isArray(parsed.samples)) {
      console.warn(`[ai-dataset-builder] Falling back to heuristic generation for batch ${index + 1}`);
      samples.push(...batch.map((mem) => buildFallbackSample(mem, index)));
      continue;
    }
    ...
    for (const item of parsed.samples) {
      ...
      const meta: Record<string, any> = { ...item.meta, batchIndex: index };
      ...
      if (original) {
        ...
        meta.maxOutputWords = OUTPUT_WORD_LIMIT;
