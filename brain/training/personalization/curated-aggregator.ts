...
async function mainWithContext() {
  ...
  let recentDays: number | undefined = undefined;
  let olderSamples: number | undefined = undefined;
  ...
  for (let i = 0; i < args.length; i++) {
    if (args[i] === '--output' && i + 1 < args.length) {
      ...
      if (['dual', 'emulation', 'agent', 'environment'].includes(mode)) {
        modeFilter = mode;
      }
    } else if (args[i] === '--days-recent' && i + 1 < args.length) {
      recentDays = parseInt(args[i + 1], 10);
    } else if (args[i] === '--old-samples' && i + 1 < args.length) {
      olderSamples = parseInt(args[i + 1], 10);
    }
  }
  ...
  if (olderSamples !== undefined && recentDays === undefined) {
    recentDays = 30;
  }
  if (recentDays !== undefined) {
    console.log(`[curated-aggregator] Rolling window: ${recentDays} recent day(s) plus ${olderSamples ?? 0} older sample(s)`);
  }
  ...
  let conversations = loadCuratedConversations(curatedDir);
  ...
  if (recentDays !== undefined) {
    const beforeWindow = conversations.length;
    conversations = selectRollingTrainingWindow(conversations, recentDays, olderSamples ?? 0);
    console.log(`[curated-aggregator] Rolling window selected ${conversations.length}/${beforeWindow} curated conversations`);
  }
...
async function main() {
  ...
  if (!username) {
    ...
    console.error('\nUsage: tsx brain/training/personalization/curated-aggregator.ts --username <username> --output <path> [options]');
    console.error('\nOptions:');
    console.error('  --max <count>           Maximum samples to generate');
    console.error('  --mode <mode>           Filter by cognitive mode (dual|emulation|agent|environment)');
    console.error('  --days-recent <days>    Include every record from the recent rolling window');
    console.error('  --old-samples <count>   Add up to this many records distributed across older history');
    console.error('\nExample:');
    console.error('  tsx brain/training/personalization/curated-aggregator.ts --username greggles --output out.json --max 5000');
    process.exit(1);
  }
